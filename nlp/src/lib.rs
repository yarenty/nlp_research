...
pub fn run_nlp() -> Result<(), Box<dyn Error>> {
    let filename = "/opt/rust_projects/nlp_research/nlp/src/model/model.pb"; // z = x + y
    ...
    println!("Create input variables for our addition");
    let mut x = Tensor::new(&[1]);
    x[0] = 2i32;
    let mut y = Tensor::new(&[1]);
    y[0] = 40i32;

    println!("Load the computation graph defined by addition.py.");
